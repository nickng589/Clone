...
public class NPCBehavior2 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (!PlayerPrefs.HasKey("Romy"))
        {
            PlayerPrefs.SetInt("Romy", 0);
        }
        Debug.Log(PlayerPrefs.GetInt("Leon"));
    }
    ...
    IEnumerator Example()
    {
        ...
        {
            case 2:
                int leon = PlayerPrefs.GetInt("Leon");
                if (leon <= -1)
                {
                    dm.DisplayText("Hey that last set of puzzles was really fun wasn’t it!");
                    ...
                    switch (dm.GrabInput(3))
                    {
                        case ChoiceSystem.Choices.Zero:
                            ...
                            PlayerPrefs.SetInt("Hazel", PlayerPrefs.GetInt("Leon") + 1);
                            break;
                ...
                {
                    dm.DisplayText("Hey it’s you again!");
                    yield return new WaitWhile(() => Input.anyKeyDown == false);
                    yield return new WaitWhile(() => !Input.GetKeyDown("space"));
